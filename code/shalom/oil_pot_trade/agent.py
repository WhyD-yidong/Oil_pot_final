...
class Agent:
    ...
    def decide_trading_unit(self, confidence):
        ...
        return (confidence / self.environment.get_price())
    ...
    def act(self, action, confidence):
        ...
        if action == Agent.ACTION_BUY:
            # 매수할 단위를 판단
            trading_unit = self.decide_trading_unit(self.balance)
            balance = (
                self.balance - curr_price * (1 + self.TRADING_CHARGE) * trading_unit
            )
            #print(trading_unit, balance)
            ...
            #print(invest_amount, trading_unit)
            if invest_amount > 0:
                self.avg_buy_price = \
        ...
        # 매도
            ...
            # 매도할 단위를 판단
            trading_unit = self.num_stocks#self.decide_trading_unit(self.balance)
