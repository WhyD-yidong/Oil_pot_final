...
class Visualizer:
    ...
    def prepare(self, chart_data, title):
        ...
        with lock:
            ...
            self.fig, self.axes = plt.subplots(
                nrows=6, ncols=1, facecolor='w', sharex=True)
            for ax in self.axes:
                # 보기 어려운 과학적 표기 비활성화
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None,
            ...
            initial_balance=None, pvs=None, lr=None, loss=None, reward=None):
        with lock:
            ...
            # 차트 6. loss
            self.axes[5].plot(loss[-2048:], '-k')
            # ax5 = self.axes[5].twinx()
            # ax5.plot(reward[-2048:], '-r')


            # 에포크 및 탐험 비율
            self.fig.suptitle(f'{self.title}\nEPOCH:{epoch_str}/{num_epoches} LR:{lr} EPSILON:{epsilon:.2f}' )
            # 캔버스 레이아웃 조정
